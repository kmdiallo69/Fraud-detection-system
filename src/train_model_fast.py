#!/usr/bin/env python3
"""
Fraud Detection System - Fast Test Version
=========================================
...
- Hyperparameter tuning (reduced for speed)
...
def perform_eda(df):
    ...
    fig.suptitle(
        "Comprehensive EDA - Credit Card Fraud Detection",
        fontsize=16,
        fontweight="bold",
    )
    ...
    for i, v in enumerate(df["Class"].value_counts().sort_index()):
        ax1.text(
            i, v + total * 0.01, f"{v/total*100:.2f}%", ha="center", fontweight="bold"
        )
    ...
    ax6.text(
        0.1,
        0.8,
        f'Mean range: {feature_stats.loc["mean"].min():.3f} to {feature_stats.loc["mean"].max():.3f}',
        fontsize=10,
    )
    ax6.text(
        0.1,
        0.7,
        f'Std range: {feature_stats.loc["std"].min():.3f} to {feature_stats.loc["std"].max():.3f}',
        fontsize=10,
    )
    ax6.text(
        0.1,
        0.6,
        f'Min range: {feature_stats.loc["min"].min():.3f} to {feature_stats.loc["min"].max():.3f}',
        fontsize=10,
    )
    ax6.text(
        0.1,
        0.5,
        f'Max range: {feature_stats.loc["max"].min():.3f} to {feature_stats.loc["max"].max():.3f}',
        fontsize=10,
    )
...
def preprocess_data(df):
    ...
    df_processed["amount_time_interaction"] = (
        df_processed["Amount"] * df_processed["Time"]
    )
    ...
    outliers_count = (
        (df_processed["Amount"] < lower_bound) | (df_processed["Amount"] > upper_bound)
    ).sum()
    print(
        f"📊 Found {outliers_count} outliers in Amount ({outliers_count/len(df_processed)*100:.2f}%)"
    )

    # Cap outliers instead of removing them
    df_processed["Amount_capped"] = df_processed["Amount"].clip(
...
# =============================================================================
# 6. 🤖 MODEL TRAINING AND EVALUATION (FAST VERSION)
# =============================================================================


def train_models_fast(X_train, y_train, X_test, y_test):
    """
    Train multiple models with reduced hyperparameter search for speed.
    ...
    """
    print("\n" + "=" * 60)
    print("🤖 MODEL TRAINING AND EVALUATION (FAST VERSION)")
    print("=" * 60)

    models = {}
    results = {}

    # Define models to train with reduced hyperparameter search
    model_configs = {
        "XGBoost": {
            "model": XGBClassifier(
                n_estimators=100,
        ...
        "Logistic Regression": {
            ...
            "params": {"C": [0.1, 1], "penalty": ["l2"], "solver": ["liblinear"]},
        },
    }
    ...
    for name, config in model_configs.items():
        ...
        # Hyperparameter tuning with reduced CV folds
...
def plot_model_comparison(results):
    ...
    print("-" * 80)
    print(
        f"{'Model':<15} {'Accuracy':<10} {'Precision':<10} {'Recall':<10} {'F1':<10} {'AUC':<10}"
    )
    print("-" * 80)
    for model, metrics in results.items():
        print(
            f"{model:<15} {metrics['accuracy']:<10.4f} {metrics['precision']:<10.4f} "
...
def save_model(model, scaler, feature_columns, model_name, results):
    ...
    print(
        f"   Performance: F1={results[model_name]['f1']:.4f}, AUC={results[model_name]['auc']:.4f}"
    )
    print(f"   Features: {len(feature_columns)}")
    print(
        f"   Files created: {model_filename}, {scaler_filename}, {features_filename}, {metadata_filename}"
    )
...
def main():
    ...
    """
    print("🚀 FRAUD DETECTION SYSTEM - FAST TEST VERSION")
    print("=" * 60)
    ...
    try:
        ...
        # 8. Train models (fast version)
        models, results = train_models_fast(
