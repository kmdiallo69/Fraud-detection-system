...
class DataPreprocessor:
    ...
    def _create_eda_plots(self):
        ...
        ax4 = axes[1, 0]
        sns.boxplot(
            data=df, x="Class", y="Amount", ax=ax4, palette=["#2E8B57", "#DC143C"]
        )
        ...
        ax6.axis("off")
        ax6.text(
            0.1, 0.9, "Feature Statistics Summary:", fontsize=12, fontweight="bold"
        )
        ax6.text(
            0.1,
            0.8,
    ...
    def engineer_features(self):
        ...
        print(
            f"📊 Found {outliers_count} outliers in Amount ({outliers_count/len(df)*100:.2f}%)"
        )
    ...
    def handle_class_imbalance(self, X_train, y_train, method="smote"):
        ...
        print(
            f"   Balance ratio: {sum(y_resampled == 0) / sum(y_resampled == 1):.2f}:1"
        )
