...
class FraudDetectionMonitor:
    ...
    def run_dashboard(self):
        ...
        try:
            ...
            st.title("🕵️ Fraud Detection System Monitor")
            st.write(
                "⚠️ Some dashboard components failed to load, but the system is working."
            )
    ...
    def setup_sidebar(self):
        ...
        self.api_url = st.sidebar.text_input(
            "API URL", value=self.api_url, help="URL of the fraud detection API"
        )
    ...
    def show_model_metrics(self):
        ...
        if self.predictor:
            ...
            st.metric(
                label="Model Type", value=model_type, delta=f"{feature_count} features"
            )
        else:
            st.metric(
                label="Model Type",
    ...
    def show_api_status(self):
        ...
        if status == "healthy":
            st.metric(
                label="API Status", value="🟢 Healthy", delta="All systems operational"
            )
        else:
            st.metric(
                label="API Status",
    ...
    def show_model_performance(self):
        ...
        with col2:
            ...
            fig.add_trace(
                go.Bar(
                    name="Accuracy", x=models, y=accuracies, marker_color="lightblue"
                )
            )

            fig.add_trace(
                go.Bar(
                    name="Precision", x=models, y=precisions, marker_color="lightcoral"
                )
            )
        ...
        if st.button("Run Batch Analysis"):
            if hasattr(self, "sample_data") and self.predictor:
                with st.spinner("Running batch analysis..."):
                    try:
                        results = self.predictor.predict_from_dataframe(
                            self.sample_data
                        )
    ...
    def show_historical_data(self):
        ...
        with col2:
            fig = px.line(
                historical_data, x="date", y="fraud_rate", title="Daily Fraud Rate"
            )
            st.plotly_chart(fig, use_container_width=True)
        ...
        with col1:
            st.metric(
                "Total Transactions", f"{historical_data['transactions'].sum():,}"
            )
        ...
        with col3:
            st.metric(
                "Avg Daily Volume", f"{historical_data['transactions'].mean():.0f}"
            )

        with col4:
            st.metric(
                "Avg Fraud Rate", f"{historical_data['fraud_rate'].mean()*100:.3f}%"
            )
